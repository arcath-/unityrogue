...
#pragma warning restore IDE0038 // Use pattern matching
    ...
    private void HandleDouble(double val, uint precision, uint maxPrecision, ulong precisionCheck)
    {
        ...
        // Write whole number first
        mMutex.WaitOne();

        int charsWrittenWhole;
        LongToBuffer(whole, (uint)(val < 0 ? FORCE_NEGATIVE : 0), out charsWrittenWhole);

        for (int i = mBuf.Length - charsWrittenWhole; i < mBuf.Length && mLength < mCapacity; i++, mLength++)
        {
            mChars[mLength] = mBuf[i];
        }
        ...
        {
            fraction *= System.Math.Pow(10, precision);
